...
export default function ProductionLineTable({ rows, onView, onSelectionPreview, description }) {
  ...
  const filters = [{ key: 'name', label: 'Production Line' }, { key: 'status', label: 'Status' }, { key: 'vendor', label: 'SW Vendor' }].map(filter => ({ ...filter, options: [...new Set(rows.map(row => row[filter.key]).filter(Boolean))].sort() }));
  const columns = [
    ...
    { key: 'status', label: 'Status', sortable: true, render: row => row.status ? <StatusBadge status={row.status} /> : <span className="muted">No switches</span> },
